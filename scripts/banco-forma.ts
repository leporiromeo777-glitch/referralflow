...
// Ogni uscita è controllata contro i tratti della forma della segretaria
...
const TRATTI: Tratto[] = [
  ...
  { nome: 'etichetta «FRCV:» nel flusso', ok: (t) => /FRCV:/.test(t) },
  { nome: 'etichetta «Comorbidità:» nel flusso', ok: (t) => /Comorbidità:/.test(t) },
  { nome: 'misure «78 Kg per 175 cm, PA 128/76 mmHg, FC 64 bpm»', ok: (t) => /78 Kg per 175 cm, PA 128\/76 mmHg, FC 64 bpm/.test(t) },
  { nome: 'etichetta «Elettrocardiogramma:»', ok: (t) => /Elettrocardiogramma:/.test(t) },
  { nome: 'FE in percentuale «FE 60%»', ok: (t) => /FE\s?60\s?%/.test(t) },
  { nome: 'data in cifre 02.09.2026', ok: (t) => /02\.09\.2026/.test(t) && !/settembre/.test(t) },
  { nome: 'conclusione «In conclusione, alla luce degli elementi di cui sopra,»', ok: (t) => /In conclusione, alla luce degli elementi di cui sopra,/.test(t) },
  { nome: 'niente istruzioni alla segreteria («copia») nel corpo', ok: (t) => !/copia anche al paziente/i.test(t) },
  { nome: 'corpo in UN SOLO paragrafo (nessun a capo fra apertura e saluto)', ok: (t) => { const r = t.split('\n'); const a = r.findIndex((x) => /^Car[oa]\s.+,$/.test(x.trim())); const c = r.findIndex((x, k) => k > a && /^Cordiali/.test(x.trim())); return a >= 0 && c > a && r.slice(a + 1, c).filter((x) => x.trim()).length === 1; } },
  { nome: 'chiusura esatta «Cordiali saluti,» e firma «Marco»', ok: (t) => /^Cordiali saluti,$/m.test(t) && /^Marco$/m.test(t) },
];
...
async function main() {
  ...
  const base: OpzioniLettera = { chiusura: m.chiusura, firma: m.firma, corpoUnico: m.corpo_lettera === 'unico' };
