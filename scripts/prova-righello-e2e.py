...
r = post({"immagine_id": ct["id"], "frame": 0, "algoritmo": "cubo", "punti": [{"x": 0, "y": 0}, {"x": 1, "y": 1}]})
...
check("strumenti dichiarati dal server nel contesto MSE", len(dd["mse"]["strumenti"]) == 10 and all(k in [x["nome"] for x in dd["mse"]["strumenti"]] for k in ["distanza", "polilinea", "angolo", "rettangolo", "ellisse", "poligono", "perimetro", "punto"]), dd["mse"].get("strumenti"))
csv = curl(f"{U}/misure?formato=csv")
