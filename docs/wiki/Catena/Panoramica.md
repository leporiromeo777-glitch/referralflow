...
Si trascinano quanti audio si vuole in **«Audio da trascrivere»** (Scrivania del Mac, percorso in `REFERTI_CARTELLA_DA_TRASCRIVERE`), direttamente o nella sottocartella del medico («Moccetti», «Moschovitis»: il servizio le crea dai profili; fuori dalle sottocartelle il dettato entra senza medico). Il servizio (`preleva_da_cartella`) ne prende **uno alla volta**, solo a catena libera (ingresso e lavorazione vuote, così i dettati dalla piattaforma non aspettano dietro la cartella) e solo a copia finita (dimensione e data ferme per due giri, ultimo cambiamento da più di 10 s: le copie dalla rete sono lente). Mentre lavora l'originale sta in «In lavorazione»; a fine catena va in **«Audio trascritti»** (`REFERTI_CARTELLA_TRASCRITTI`), oppure in «Audio trascritti/Non riusciti» se la catena l'ha messo in `errori/`. In `ingresso/` entra una copia col nome `medico-<id>--cartella-<token>.<ext>`: il nome originale, che può contenere il nome del paziente, non entra nella catena né nel log (solo estensione, medico e quanti ne restano). Stato in `~/referti/cartella-condivisa.json`; all'avvio un prelievo rimasto a metà torna nella sua cartella (`ripara_cartella`). **Spegnimento a metà** (Mac portato in studio, 23.9.2026): all'avvio del servizio l'audio rimasto in `lavorazione/` torna in `ingresso/` e si rifà da capo (`riprendi_lavorazione`, vale anche per i dettati dalla piattaforma); prima restava lì per sempre e bloccava la cartella condivisa. Gli intermedi `<file_id>.*` non contano come «catena occupata». Caso 43 nella suite. Formati: `.ds2 .dss .wav .mp3 .m4a .aac .ogg .opus .flac .wma .amr .3gp .mp4 .webm .caf .aif(f)`; gli altri file restano dove sono. Caso 41 nella suite.

**Riascolto** (23.9.2026): i dettati entrati dalla cartella (o messi a mano in `ingresso/`) non avevano l'audio nella piattaforma, e la revisione restava muta (32 bozze su 32 il primo giorno). Ora, dopo la bozza, `consegna_audio` manda il file originale a `POST /api/referti/audio-catena` (token della catena, `?file_id=&ext=`, mai il nome del file): la piattaforma lo salva, lo collega alla bozza con quell'impronta (riga in `referti_audio`, stato `fatto`, nome «dettato.<ext>») e i `.DS2` li converte per il browser come gli altri. Idempotente. Se la consegna non riesce la bozza resta buona e l'audio finisce comunque nella cassaforte locale; `pipeline-referti/collega-audio.py` ricollega dalla cassaforte quelli mancanti (usato il 23.9.2026: 33 collegati, 17 già presenti).

Condivisione con gli altri computer (anche Windows): condivisione file SMB del Mac, solo nella rete dello studio, mai cartelle sincronizzate su cloud (OneDrive, Dropbox, iCloud). Gli audio in «Audio trascritti» sono dati sanitari: il servizio li **cancella dopo 7 giorni** dall'arrivo in quella cartella (`pulisci_trascritti`, `REFERTI_CARTELLA_TRASCRITTI_GIORNI`, 0 = spenta; conta la data di arrivo, non quella della registrazione; controllo una volta l'ora; solo file audio al primo livello). «Non riusciti» non si cancella mai da sé: quei dettati non sono stati trascritti e qualcuno deve guardarli. Caso 42 nella suite.
