...
Entra in: `PROMPT_CATENA_COMPATTA` e `PROMPT_CORREZIONE_LISTA` tramite `{contesto_medico}` (`prompt_correzione()`), [[Catena/Arbitro]] (`_prompt_arbitro`), [[Catena/Coerenza interna]]. Vuoto senza profilo. Fa parte dell'impronta del prompt in `versione_catena()`: cambia il profilo, cambia la versione, e il cruscotto confronta. Caso 27 nella suite.
