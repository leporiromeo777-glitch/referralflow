...
La pagina vecchia `/referti/qualita/pipeline` (solo admin) resta per le parti che non sono state portate: filtri per versione di catena/prompt/medico, rilasci come linee verticali, outlier, attribuzione per tappa, frasi ricorrenti, proposte di dizionario da confermare, latenze. `/referti/[id]/storia`: linea del tempo, corse e tappe espandibili, versioni con provenienza, modifiche umane con diff in linea e affiancato. Test `npm run test:audit`; `npm run audit-backfill` (idempotente).
