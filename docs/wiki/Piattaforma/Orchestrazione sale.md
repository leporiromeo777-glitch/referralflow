...
**«Arrivato» vuol dire «è in sala d'attesa»** (16.9.2026 sera). Quando è una *persona* a segnare l'arrivo, lo stato che viene scritto è `in_attesa`, non `arrivato`: chi sta all'accoglienza fa un gesto solo, e il paziente compare subito nella pagina del suo medico. Lo stato `arrivato` resta per chi non è una persona — il robot legge l'agenda e dell'attesa non sa niente. La regola sta in una funzione sola, `statoDopoArrivo()`, ed è provata.

**Correggere un tasto premuto per sbaglio** (16.9.2026 sera). La macchina a stati non torna indietro da sola, ed è giusto: un sistema che disfa i propri passi non è più una traccia di quel che è successo. Ma chi ha premuto «finito» sul paziente sbagliato deve poter rimediare, e `correggiStato()` lo fa: riporta lo stato indietro, **azzera le ore che vengono dopo** (chi torna in attesa non ha più un'ora d'inizio né una stanza) e **toglie la durata che quel tasto aveva misurato** — senza, la previsione imparerebbe da un errore; il 16.9 una visita cardiologica di Marco Moccetti risultava durata un minuto. Resta scritta come correzione negli eventi, non fatta sparire. Per questo `durate_osservate` porta l'`appointment_id` (migrazione 061): senza, la misura sbagliata non era rintracciabile.

La correzione si chiedeva dalla pagina «Visita», che non c'è più: oggi l'endpoint (`POST /api/orchestrazione/eventi`, `tipo: 'correzione'`) **non ha nessun tasto**. Va rimesso dove la gente guarda davvero — accoglienza o la lista «Adesso» della pagina Sale — la prossima volta che si tocca quella parte.
...
**E. La pagina «Visite»** (16.9.2026 sera, seconda versione). La prima — «Visita», un centro di controllo pieno di cose — è stata tolta poche ore dopo essere nata: allo studio non piaceva. La seconda è nata al contrario, da un progetto disegnato e guardato prima di scrivere il codice ([[Decisioni/Registro]]). Tre cose sole:

**L'elenco.** Solo i *propri* pazienti, e solo quelli **in sala d'attesa**: nome, prestazione, ora dell'appuntamento, ora d'arrivo, e un tasto «Inizia visita» — pieno di colore sul primo, contornato sugli altri, così il tasto acceso è sempre uno solo. L'ordine è per ora di appuntamento, non per ora d'arrivo: chi arriva presto non passa avanti. Un pallino ambra e un «da 24 min» compaiono solo per chi aspetta da più di venti minuti — misurano l'attesa **del paziente**, non la velocità del medico.

**La visita.** Testata che non se ne va mai (nome, età, prestazione, «Visita in corso», e le **allergie**, promosse lì perché dentro un pannello sarebbero una cosa da ricordarsi di aprire); sotto, il **briefing**: sei righe al massimo, sempre le stesse, tutte lette dai dati e nessuna scritta da un modello — oggi, perché è qui, chi l'ha mandato, ultimo contatto, terapia in corso, già noto; una riga senza contenuto sparisce. Poi la **cartella a pannelli chiusi**, con il numero accanto e **uno aperto per volta**: le cinque voci più recenti e «vedi tutti» che porta alla cartella intera. Una cartella con trecento documenti è alta come una con tre.
...
**Quello che manca perché sia davvero «i tuoi pazienti»**: account veri. Oggi in studio si entra tutti con lo stesso, e la pagina deve chiedere una volta chi sei (la scelta resta sul dispositivo). Quando ogni medico avrà il suo utente collegato a `providers`, la domanda sparisce da sola.
