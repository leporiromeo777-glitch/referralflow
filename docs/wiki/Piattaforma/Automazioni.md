...
- Sync agenda ogni 15 min → `/api/cron/agenda` (tutti i feed attivi, tutti gli studi); a ogni giro aggancia `/api/cron/piano-sale` e `/api/cron/orchestrazione` (la baseline della giornata del gemello, una volta al giorno; `?forza=1` la rifà)
