...
Compilata in `medici.json` (profilo `moschovitis`) da `compila-conoscenza.py`: «Come detta» → `contesto`, «Frasi fisse» → `frasi_fisse`, «Farmaci frequenti» → `farmaci_frequenti`. Vedi [[Agenti/Come funziona]]. Niente dati di pazienti.

## Come detta
Detta aggiornamenti della lettera precedente del paziente, come rapporto a sezioni: diagnosi principali numerate con «- attuale:», diagnosi secondarie, comorbidità, anamnesi, terapia domiciliare, esami con date, valutazione, procedere. Cita spesso valori ecocardiografici (FE, TAPSE, LAVi, SIV/PP) e pressori.
...
Modalità aggiornamento con fusione automatica, formato rapporto. Il resto in [[Medici/Moschovitis]].
