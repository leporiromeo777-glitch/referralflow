...
### 2.1 Nessun dato esce dalla macchina
...
- L'unica connessione di rete ammessa è verso il PostgreSQL di ReferralFlow, sulla rete locale.
...
- L'audio va cancellato **solo dopo** che il referto è stato salvato con successo nel database.
- Mai prima. Se una fase intermedia fallisce, l'audio resta.
- Cancellazione sicura: sovrascrittura, non semplice unlink.
...
[10] controlli numerici          intervalli da correzioni.json → lista ALLARMI
         │
[11] salvataggio                 POST a ReferralFlow come bozza
         │
...
| Config | `correzioni.json` (già fornito) | non hardcodare le sostituzioni |
...
```json
{
  ...
  "campi_estratti": { ... },
  "divergenze": [
    { "posizione": "...", "versione_a": "...", "versione_b": "..." }
  ],
  "segmenti_dubbi": [ "..." ],
  "allarmi_numerici": [
...
**Fase 4** — Aggiungi il dizionario da `correzioni.json`.
...
**Fase 8** — Aggiungi l'invio a ReferralFlow e la cancellazione condizionata dell'audio.
