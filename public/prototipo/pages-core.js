...
PAGES.patient = () => {
  ...
  const head = `
    <div class="row between wrap mb-16" style="align-items:flex-start">
      <div class="row" style="gap:16px"><div class="avatar" style="width:56px;height:56px;font-size:18px">${initials(p)}</div><div><h2 class="page-title">${fullName(p)}</h2><div class="page-sub num">${p.age ? `${p.age} anni · ` : ''}${p.sex ? `${p.sex} · ` : ''}${p.dob ? `nato/a il ${p.dob} · ` : ''}${p.num ? `ID ${p.num} · ` : ''}${p.phone ? `${p.phone} · ` : ''}${DOCTORS[p.doctor] || '—'}</div><div class="row wrap mt-8">${p.flags.map(f => `<span class="badge warning">${ICONS.alert} ${f}</span>`).join('')}${p.docs.some(d => d.new) ? '<span class="badge accent">Documenti nuovi</span>' : ''}</div></div></div>
