...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  // L'allegato è visibile solo allo studio destinatario del consulto.
  const [att] = await query<{ storage_key: string; filename: string }>(
    `select a.storage_key, a.filename
       from consulto_attachments a
       join consulti c on c.id = a.consulto_id
      where a.id = $1 and c.studio_id = $2`,
